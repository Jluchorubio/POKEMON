// Fallback to global fetch (Node 18+) if node-fetch is not installed
let _fetch;
try {
  _fetch = globalThis.fetch.bind(globalThis);
} catch (e) {
  // If no global fetch, try node-fetch dynamic import (only if installed)
  _fetch = (...args) => import('node-fetch').then(mod => mod.default(...args));
}
const fetch = _fetch;
